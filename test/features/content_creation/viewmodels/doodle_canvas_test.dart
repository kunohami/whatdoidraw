...
class MockContentCreationService extends Mock
    implements ContentCreationService {}
...
class MockAuthController extends AsyncNotifier<User?>
    with Mock
...
void main() {
  ...
  group('DoodleCanvas ViewModel Tests', () {
    ...
    test(
      'submitDoodle debe actualizar isSubmitting y llamar al servicio',
      () async {
        final notifier = container.read(doodleCanvasProvider.notifier);
        ...
        when(
          () => mockService.insertDoodle(any(), any(), any()),
        ).thenAnswer((_) async => {});
        ...
        verify(
          () => mockService.insertDoodle(any(), 'test-user-id', null),
        ).called(1);
      },
    );

    test(
      'submitDoodle debe capturar errores y guardarlos en el estado',
      () async {
        final notifier = container.read(doodleCanvasProvider.notifier);
